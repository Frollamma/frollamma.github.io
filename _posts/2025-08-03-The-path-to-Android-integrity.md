...
- If something didn't work (or stopped working after some time), consider trying again adding these steps and try again:
  ...
  - If you just got Basic Integrity, it might be because the YuriKey module is not always up to date with the latest working keybox, in this case you might want to ask on [Yuri's Telegram group](https://t.me/yurichattt) for the latest keybox, sometimes there's no keybox and you need to wait. In this case you need to run the action for Tricky Store and a GUI will open up (otherwise go on the app KsuWebUI and select Tricky Store, it should have been installed automatically when you installed Tricky Store), press on the hamburger menu, you should see a `Set Custom Keybox` option, and just upload your keybox.
